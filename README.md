...
Each workday it invites the team in a Slack channel, reads who's free and when, matches everyone into pairs — a trio when the headcount is odd, so nobody's left out — rotates the pairings day to day, sends each group a Google Calendar invite with a Meet link, and posts each match right in Slack.

> **Status:** v0.5.1 — timezone-aware and hands-off on a schedule once set up; durable state lives in Slack canvases, with Google Calendar invites layered on top.
...
- **Real calendar invites.** One Google Calendar event per match, with a **Google Meet** attached for remote folks. The bot adds *itself* (the organizer) as an optional attendee — it schedules lunch, it doesn't eat it. The in-channel Slack match message announces the invite, with an optional static meeting link (your Zoom/Meet personal room) as a secondary room and an optional at-slot reminder when lunch is starting.
- **Resilient state, graceful degradation.** Durable state — config, roster, availability, history — lives in **Slack canvases** (overwrite-in-place, server-side in the workspace), so it survives a plugin update and the ephemeral Cowork sessions the scheduled runs fire in; there's no Google Drive in the loop. If Google ever breaks again, only the invites degrade — the state, the matching, and the Slack notifications keep working.
...
flowchart TB
    ...
    subgraph TRUSTED["🔒 Trusted zone · every decision, all canvas state + Calendar writes"]
    ...
    orch -->|"spawn NOTIFY: announce invite in channel<br/>(+ optional meeting link + at-slot reminder)"| msgr
    msgr <--> chan
    orch -.->|"setup only: create_conversation"| chan
    orch -->|"overwrite canvas (replace)"| canvas
    canvas -->|"find by sentinel + read"| orch
    orch -->|"create_event · Meet + organizer optional"| cal
...
sequenceDiagram
    ...
    alt two or more are due this run
        ...
        loop each group (skip if event already exists today)
            Orch->>Cal: create_event + Google Meet (organizer = optional)
        end
        Orch->>Msgr: NOTIFY — announce invite (slot in each zone, + optional link + reminder)
...
| **Google Calendar** | the orchestrator | creating the lunch events (one per match, with a Google Meet) |
...
3. **Run first-time setup:**
   ...
   /lunch setup
   ```
   It walks you through confirming the Slack workspace, creating (or pointing at) the intake channel, and capturing the host timezone, the organizer/calendar email (the account that owns and sends the invites), the lunch window, the run schedule, and an optional meeting link (a static Zoom/Meet room URL added to each match message as a secondary room). It seeds `config.json` and starts with an empty roster that fills from channel membership. The state canvases are created automatically on the first write — nothing to pre-create.
4. **Schedule the daily runs** — hourly across the team's morning; see [`references/scheduling.md`](skills/lunch-roulette/references/scheduling.md). Each run is then simply:
   ...
   ```
   which syncs the channel, pairs whoever's lunch is due, sends the calendar invites, and posts the matches in Slack.
...
| `organizer_email` / `calendar_id` | the account that owns and sends the Google Calendar invites (added as an optional attendee; the event-existence check and create both run on `calendar_id`) |
| `meeting_link` | *optional* static video-call URL (a Zoom/Meet personal room) added to every match message as a secondary room; the calendar invite's Google Meet is the primary join link, so empty is fine |
...
```bash
git archive --format=zip -o lunch-roulette-v0.5.1.plugin HEAD
```
